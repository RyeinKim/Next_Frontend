...
/*    useEffect(() => {
        const getPostByReply = async () => {
            try {
                const res = await fetch (`${apiUrl}/reply/byUser/all/${userId}?offset=0&limit=5`, {
                    method: 'GET',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                    },
                ...
                /!* data.message 로 반환된 값 중 data.postId 값을 통해 게시글 리스트를 나열할거임 *!/
    ...
    const fetchPosts = async (pageNumber: number) => {
        ...
        try {
            const res = await fetch(`${apiUrl}/reply/byUser/all/${userId}?offset=${offset}&limit=${postsPerPage}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`,
                                ...
                                {posts.map(post => (
                                    <div key={post.postId} className={styles.post}>
                                        {/*<Link href={`/board/viewPost/notice/${post.postId}`}>*/}
                                            <span className={styles.postHead}>{post.reply}</span>
                                            <span className={styles.postBody}>{post.username}</span>
                                        {/*</Link>*/}
                                    </div>
                                ))}
                            </div>
