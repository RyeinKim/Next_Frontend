...
export default function Free() {
    ...
    useEffect(() => {
        ...
        const fetchPostCount = async () => {
            try {
                ...
                const res = await fetch(`${apiUrl}/posts/getPost/free`); // 적절한 API 엔드포인트로 변경하세요
    ...
    const fetchPosts = async (pageNumber: number) => {
        ...
        try {
            const apiUrl = process.env.NEXT_PUBLIC_API_URL;
            const res = await fetch(`${apiUrl}/posts/getPost/free?offset=${offset}&limit=${postsPerPage}`);
            const data = await res.json();
            ...
            <div id={styles.wrapper}>
                <div className={styles.flexContainer}>
                    <div id={styles.sectionDiv}>
                        <section>
                            <h2>공지사항</h2>
                            <Link href="/board/writePost/free">
                                <button>게시글 작성</button>
                            </Link>
                            <div id="board-list">
                                {posts.map(post => (
                                    <div key={post.postId} className={styles.post}>
                                        <Link href={`/board/viewPost/free/${post.postId}`}>
