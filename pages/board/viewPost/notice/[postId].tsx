...
const PostId = () => {
    ...
    const fetchReply = async () => {
        try {
            const res = await fetch(`${apiUrl}/reply/getPost/notice/${postId}`);
    ...
    const handleReplySubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const token = Cookie.get('accessToken');
            // 백엔드로 데이터 전송
            const response = await fetch(`${apiUrl}/reply/write/notice`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
    ...
    const handleEditSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const token = Cookie.get('accessToken');
            // 백엔드로 데이터 전송
            const response = await fetch(`${apiUrl}/posts/edit/notice`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const handleDelete = async () => {
        ...
        if (check) {
            try {
                const token = Cookie.get('accessToken');
                // 백엔드로 데이터 전송
                const response = await fetch(`${apiUrl}/posts/delete/notice/${postId}`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json',
    ...
    const handleForcedDelete = async () => {
        ...
        if (check) {
            try {
                const token = Cookie.get('accessToken');
                // 백엔드로 데이터 전송
                const response = await fetch(`${apiUrl}/admin/posts/delete/notice/${postId}`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json',
    ...
    const handleReplyEditSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const token = Cookie.get('accessToken');
            // 백엔드로 데이터 전송
            const response = await fetch(`${apiUrl}/reply/edit/notice/${editingReplyId}`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const handleReplyDelete = async (replyId: number) => {
        ...
        if (check) {
            try {
                const token = Cookie.get('accessToken');
                // 백엔드로 데이터 전송
                const response = await fetch(`${apiUrl}/reply/delete/notice/${replyId}`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                    },
